...
void Game::render_coord()
{
    coordText.str("");
    coordText<<playerList[0].getX()<<' '<<playerList[0].getY();
    SDL_Color yellowColor = { 252,226,5,255 };
    SDL_Texture* coordTex = commonFunc::createText(coordText.str(),yellowColor);
    Entity coordTextEntity(1130,690,coordTex);
    commonFunc::renderTexture(coordTextEntity);
